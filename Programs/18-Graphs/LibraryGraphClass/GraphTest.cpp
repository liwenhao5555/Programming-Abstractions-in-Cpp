...
void runFindCycleCommand(TokenScanner & scanner, void *clientData);
...
Path findCycle(Graph<Node,Arc> & g);
...
void initCommandScanner(CommandScanner & cs) {
   ...
   cs.define("findCycle", runFindCycleCommand,
             "findCycle -- Finds a cycle in the graph, if any");
}
...
void runFindCycleCommand(TokenScanner & scanner, void *clientData) {
   Graph<Node,Arc> *gp = (Graph<Node,Arc> *) clientData;
   Path path = findCycle(*gp);
   if (path.isEmpty()) {
      cout << "No cycles exist" << endl;
   } else {
...
Path findCycle(Graph<Node,Arc> & g) {
   foreach (Node *start in g.getNodeSet()) {
      foreach (Arc *arc in start->arcs) {
         Node *first = arc->finish;
         Path path = findShortestPath(first, start, false);
         if (!path.isEmpty()) {
            Path result;
            result.extend(arc);
            int n = path.size();
            for (int i = 0; i < n; i++) {
               result.extend(path[i]);
            }
            return result;
         }
      }
